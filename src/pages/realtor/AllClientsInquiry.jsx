...
import { useViewInquiries } from "../../hooks/useInquiry";
...
export default function AllClientsInquiry() {
  const { user } = useUser();
  ...
  const { isPending, inquiries, isError, totalCount } = useViewInquiries({
    sort,
    search: activeSearch,
    page: currentPage,
  });
  ...
  return (
    <>
      ...
      <GoBackBtn />
      <IntroHeading label={`View all prospects (${inquiries?.length || 0})`} />
      <SearchAndSort
        search={search}
                ...
                {inquiries &&
                  inquiries.map((el, idx) => (
                    <tr key={idx}>
                      ...
                      <td>
                        <Link
                          to={user.role !== "admin" ? "/app/live-chat" : null}
